...
touch task2.out.all
echo "Run Total_Rel Avg_Last_Rel MAP R@5% R@10% R@20% R@30% WSS95 WSS100 Reliability R@k k" > task2.out.all
...
for filename in `find -f * | grep "task2$"`; do
   ...
   echo "${filename} ${hashmap["num_rels"]} ${hashmap["last_rel"]} ${hashmap["ap"]} ${hashmap["recall@5.0%"]} ${hashmap["recall@10.0%"]} ${hashmap["recall@20.0%"]} ${hashmap["recall@30.0%"]} ${hashmap["wss_95"]} ${hashmap["wss_100"]} ${hashmap["loss_er"]} ${hashmap["recall_threshold"]} ${hashmap["threshold"]}"  >> task2.out.all
